...
def _adaptive_bin_initializer(bin_dict, num_bins=10):
    for i in range(num_bins):
        bin_dict[i][COUNT] = 0
        bin_dict[i][CONF] = 0
        bin_dict[i][ACC] = 0
...
def _populate_bins_adaptive(confs, preds, labels, num_bins=10):
    """
    Populate bins adaptively so each bin has equal number of samples
    """
    bin_dict = {}
    for i in range(num_bins):
        bin_dict[i] = {}
    _adaptive_bin_initializer(bin_dict, num_bins)
    
    num_test_samples = len(confs)
    # Sort confidences and create adaptive bin boundaries
    sorted_indices = np.argsort(confs)
    samples_per_bin = num_test_samples // num_bins
    ...
    for i in range(num_test_samples):
        ...
        label = labels[idx]
        binn = min(i // samples_per_bin, num_bins - 1)
...
def adaptive_expected_calibration_error(confs, preds, labels, num_bins=10):
    """
    Compute Adaptive ECE where bins are created to have equal number of samples
    """
    bin_dict = _populate_bins_adaptive(confs, preds, labels, num_bins)
    num_samples = len(labels)
    ece = 0
    for i in range(num_bins):
        bin_accuracy = bin_dict[i][BIN_ACC]
